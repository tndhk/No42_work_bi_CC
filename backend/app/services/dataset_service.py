...
class DatasetService:
    ...
    async def import_csv(
        ...
        return await self._process_and_save_csv(
    ...
    async def _process_and_save_csv(
        self,
        file_bytes: bytes,
        name: str,
        owner_id: str,
        ...
        """Process CSV bytes and save as dataset.
        ...
        Raises:
            ValueError: If CSV is empty or could not be parsed
        """
        # Generate dataset ID
        ...
        return await self._save_metadata(
            dataset_id=dataset_id,
            name=name,
            owner_id=owner_id,
            df=df,
    ...
    async def reimport_dry_run(
        ...
        """
        dataset, df, new_schema = await self._prepare_reimport(
            dataset_id, dynamodb, source_s3_client
        )
    ...
    async def _prepare_reimport(
        self,
        dataset_id: str,
        dynamodb: Any,
        source_s3_client: Any,
    ) -> tuple[Dataset, pd.DataFrame, list[Any]]:
        """Prepare reimport by fetching dataset, CSV, and inferring schema.
        ...
        Returns:
            Tuple of (Dataset, DataFrame, new_schema)
