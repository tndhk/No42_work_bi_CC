"""Dashboard repository for DynamoDB operations."""
...
class DashboardRepository(BaseRepository[Dashboard]):
    """Repository for Dashboard entity operations.

    Provides CRUD operations and owner-based listing using GSI.
    """

    def __init__(self) -> None:
        """Initialize DashboardRepository with dashboards table configuration."""
        super().__init__(
            table_name=f"{settings.dynamodb_table_prefix}dashboards",
            pk_name='dashboardId',
            model=Dashboard
        )

    async def list_by_owner(self, owner_id: str, dynamodb: Any) -> list[Dashboard]:
        """Retrieve all dashboards owned by a specific user.
