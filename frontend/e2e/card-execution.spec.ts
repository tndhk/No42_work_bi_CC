...
test.describe('Card Execution & Preview', () => {
  ...
  test('CARD-EXEC-01: カードプレビュー実行で結果が表示される', async ({ page }) => {
    ...
    // Then: プレビュー結果が表示される (iframeの読み込みを待機)
    const previewIframe = page.locator('iframe[title="card-preview"]');
    await expect(previewIframe).toBeVisible({ timeout: 30000 });

    // Then: iframeの内容にデータが含まれることを確認
    const frame = page.frameLocator('iframe[title="card-preview"]');
    // テーブルまたはデータが表示されていることを確認
    await expect(
      frame.locator('body').or(frame.locator('table')).or(frame.locator('div'))
  ...
  test('CARD-EXEC-03: カード新規作成時にコード未入力で保存するとエラー', async ({ page }) => {
    ...
    await expect(page.getByText('新規カード作成')).toBeVisible();
  ...
  test('CARD-EXEC-05: カードプレビューで結果テーブルが正しく表示される', async ({ page }) => {
...
df`;
    ...
    // Then: プレビュー結果のiframeが表示される
    const previewIframe = page.locator('iframe[title="card-preview"]');
    await expect(previewIframe).toBeVisible({ timeout: 30000 });

    // Then: iframeの中にコンテンツが存在することを確認
    const frame = page.frameLocator('iframe[title="card-preview"]');
    await expect(frame.locator('body')).not.toBeEmpty({ timeout: 30000 });
  });

  test('CARD-EXEC-06: カード編集後にプレビューが更新される', async ({ page }) => {
    // Given: データセットとカードが作成済み
    const timestamp = Date.now();
  ...
  test('CARD-EXEC-07: カード作成からプレビューまでの完全フロー', async ({ page }) => {
    ...
    await page.getByLabel('カード名').fill(cardName);
    await page.getByLabel('説明').fill('E2E test card for flow test');
